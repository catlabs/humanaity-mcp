...
export function registerCityTools(server: McpServer, backendClient: BackendClient): void {
  server.tool(
    "cities_list",
  ...
  server.tool(
    ...
    async ({ accessToken }) => {
      try {
        ...
        return {
          content: [{ type: "text", text: `Fetched ${cities.length} owned cities.` }],
          structuredContent: { ok: true, cities },
        };
      } catch (error: unknown) {
        const normalized = toToolError(error);
  ...
  server.tool(
    ...
    async ({ name, accessToken }) => {
      try {
        ...
        return {
          content: [{ type: "text", text: `City created: ${city.name} (#${city.id}).` }],
